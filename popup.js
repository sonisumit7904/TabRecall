...
async function loadCurrentTabs() {
  ...
  tabsList.innerHTML = '<div class="empty-state"><div class="loading"></div><div>Loading tabs...</div></div>';
  ...
  try {
    ...
    for (const tab of tabs) {
      let tabData;
      try {
        tabData = await chrome.runtime.sendMessage({ action: "summarizeTab", tab });
      } catch (error) {
        console.warn('No summary available for tab, using default tab info:', error);
        tabData = tab;
      }
    ...
    tabsList.innerHTML = '<div class="empty-state"><div class="empty-state-icon">⚠️</div><div class="empty-state-title">Error Loading Tabs</div><div class="empty-state-message">Please try again later</div></div>';
  }
}
...
function createTabElement(tab) {
  ...
  // Add event listener to show full summary in a popup
...
async function loadSavedWorkspaces() {
  ...
  container.innerHTML = '<div class="empty-state"><div class="loading"></div><div>Loading workspaces...</div></div>';
  ...
  try {
    ...
    container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">⚠️</div><div class="empty-state-title">Error Loading Workspaces</div><div class="empty-state-message">Please try again later</div></div>';
  }
}
...
function createWorkspaceElement(workspace) {
  ...
  // Add event listener to show full workspace summary when summary is clicked
  summary.addEventListener('click', (e) => {
    e.stopPropagation();
    showFullWorkspaceSummary(workspace);
  });
  ...
  workspace.tabs.forEach(tab => {
    ...
    tabItem.innerHTML = `
      <img src="${tab.favIconUrl || 'icons/default-favicon.png'}" alt="" class="tab-favicon">
      <div class="tab-details">
        <div class="tab-title">${tab.title}</div>
        <div class="tab-summary">${tab.summary || ''}</div>
      </div>
    `;
    // Add event listener to display full summary popup when saved workspace tab is clicked
  ...
  meta.textContent = `${workspace.tabs.length} tabs • ${new Date(workspace.created).toLocaleDateString()}`;
  
  // Calculate some quick stats
  const uniqueDomains = new Set(workspace.tabs.map(tab => {
    try {
...
function setupEventListeners() {
    ...
    try {
      ...
      const summarizedTabs = await Promise.all(
        currentTabs.map(async tab => {
          try {
            const tabData = await chrome.runtime.sendMessage({ action: "summarizeTab", tab });
            return tabData || {
              url: tab.url,
              title: tab.title,
            ...
            console.warn('No summary available for tab, using fallback info:', error);
            return {
              url: tab.url,
              title: tab.title,
              favIconUrl: tab.favIconUrl,
  ...
  // Search workspaces
  document.getElementById('search-workspaces').addEventListener('input', async (e) => {
  ...
  document.getElementById('workspaces-container').addEventListener('click', async (e) => {
